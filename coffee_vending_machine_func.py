...
def select_bevarage(): # 음료 선택 함수
    ...
    if selected_number == 1: # 아메리카노 선택 시
        ...
        return select_bevarage() # 재귀 호출로 다시 음료 선택 함수 실행
...
def make_bevarage(): # 음료 제조 함수
    ...
    if is_ice:
        if ingredient["ice"] < 30: # 얼음 30g 소모 가정
            print("얼음 재고가 부족합니다. 다른 음료를 선택해주세요.")
            return make_bevarage()

    pay_money(pick_bevarage) # 결제 함수 호출
    ...
    if is_ice:
        ingredient["ice"] -= 30
        print(f"아이스 {pick_bevarage}가 완성되었습니다. 맛있게 드세요!") # 음료 완성 메시지 출력
    else:
        print(f"따뜻한 {pick_bevarage}가 완성되었습니다. 맛있게 드세요!") # 음료 완성 메시지 출력
...
def continue_or_not(): # 계속 진행 여부 함수
    ...
    if user_input.lower() == 'y': # 'y' 입력 시
        ...
        return continue_or_not() # 재귀 호출로 다시 계속 진행 여부 함수 실행
