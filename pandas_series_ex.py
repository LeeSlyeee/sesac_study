...
# print(s)
# # dtype: float64
# # 판다스가 처리하는 자료구조인 시리즈와 데이터프레임에서 결측치가 있는 경우는 datatype이 float으로 변환한다


# # 인덱스를 지정해 주면 지정된 인덱스로 표현해준다.
# s=pd.Series([10,20,30],index=[1,2,3]) 
# print(s)


# # 인덱스를 문자열로 지정해 줄 수도 있다.
...
# print(s0.index)  #Int64Index([1, 2, 3], dtype='int64')
...
# print(s.index) # 문자열형 인덱스. Index(['서울', '부산', '인천', '대구'], dtype='object')
...
# # "Integer Location"을 명시하여 2번째 위치의 값을 가져옴. 숫자로 순서를 따질 때는 무조건 iloc를 쓰는 습관을 들이는 것을 권장
# print(s.iloc[2]) 


# # 0번째와 3번째 값을 둘 다 보고 싶을 때 리스트 형태로 감싸서 전달해야 합니다.
# print(s.iloc[[0, 3]])


# # 대괄호 안에 쉼표로 숫자만 나열하면 튜플 (0, 3, 1)이라는 '하나의 키'를 찾는 것으로 인식합니다. 하지만 인덱스에 (0, 3, 1)이라는 이름은 없으므로 KeyError가 발생합니다.
# print(s[0,3,1])


# # 시리즈명[[인덱스리스트]] - 시리즈형태로 반환 - FutureWarning
# print(s[[0,3,1]]) #- 인덱스 리스트 내의 해당 인덱스의 item을 추출 후 시리즈 형태로 반환
...
# # 그리고 이전에 생성한 시리즈 s의 인덱스(s.index, 즉 서울, 부산, 인천, 대구)를 그대로 재사용하여 새로운 시리즈 s1을 생성합니다.
...
# # 결과: 250만 초과, 500만 미만에 해당하는 '부산(3,448,737)'과 '대구(2,466,052)'가 추출되어야 하지만, 대구는 250만보다 작으므로 조건을 만족하지 못합니다. (수정: 246만은 250만보다 작으므로 대구는 제외되어야 합니다. s > 250e4 조건에 의해 대구는 False입니다.)
...
# # 너무 위에 있어서 보기 편하라고 71번라인거 그대로 다시 씀.  
s= pd.Series([9904312,3448737,289045,2466052], index=["서울","부산","인천","대구"])
